...
fun MoreSheet(
  ...
  PlayerSheet(
    ...
    Column(
      ...
      // 超分档位选择
      Text(text = "超分档位")
      val superResLevel by viewModel.superResLevel.collectAsState()
      LazyRow(
        horizontalArrangement = Arrangement.spacedBy(MaterialTheme.spacing.smaller),
      ) {
        items(listOf("关闭" to 0, "质量档" to 1, "效率档" to 2)) { (label, level) ->
          FilterChip(
            selected = superResLevel == level,
            onClick = { viewModel.setSuperResLevel(level) },
